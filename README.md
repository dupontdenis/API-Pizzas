...
- **Purpose:** The `public/` folder contains a tiny static frontend used to manually test the API. When the server is running it serves the `public` directory as static files so a request to `/` will return `public/index.html` and the browser will load the client-side JavaScript.
- **How it finds `index.html`:** The server exposes the `public/` directory (for example via `express.static('public')` in `app.js`), so visiting `http://localhost:3000/` returns `public/index.html`. The frontend then issues `fetch` requests to the API endpoints (see next point).
- **How the frontend fetches the API:**
  - `public/main.js` uses a configurable `API_BASE` (default `http://localhost:3000`) and calls `fetch(`${API_BASE}/pizzas`)` and `fetch(`${API_BASE}/toppings`)` when you click the buttons.
  - `public/pizza.js` and `public/topping.js` use a helper `fetchJsonPath(path)` that first tries the current origin (so it works when the backend serves the files) and falls back to `http://localhost:3000` — useful when previewing with a static server or Live Server.

**Files (quick reference)**

- `public/index.html`: The single-page tester UI. It contains buttons to load pizzas and toppings and two lists where results are rendered. It loads `public/main.js` as a module.
- `public/main.js`: Controls the index page. Key parts:
  - `API_BASE`: the base URL for API calls; change this if the server runs on another host/port.
  - `loadPizzas()` and `loadToppings()` perform `fetch()` to the API endpoints, validate JSON responses and render lists.
  - Clicking a pizza/topping link opens `pizza.html` / `topping.html` with `?id=...`.
- `public/pizza.html` + `public/pizza.js`: Detail view for a pizza. `pizza.js` reads the `id` query param and calls `/pizzas/:id`. It renders pizza details and the list of toppings (each links to `topping.html?id=...`). The script prefers the current origin and falls back to `http://localhost:3000`.
- `public/topping.html` + `public/topping.js`: Detail view for a topping. `topping.js` fetches `/toppings/:id` and also attempts `/toppings/:id/pizzas` to show pizzas that include that topping.
- `public/style.css`: Simple styles for the tester UI.

**Quick tips**

- If you run the server and visit `http://localhost:3000/` and the lists show errors, check the browser console and ensure the backend is running on the same origin (or edit `API_BASE` in `public/main.js`).
- To preview the static site without the backend, use a static server (e.g. Live Server). The fallback logic in `pizza.js` / `topping.js` will try `http://localhost:3000` for API calls.
